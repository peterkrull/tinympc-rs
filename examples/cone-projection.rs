...
use rerun::Color;
use tinympc_rs::{
    Error, CircularCone, project::Project
};
...
fn main() -> Result<(), Error> {
    ...
    // Velocity limiter
    let mut cone_project = CircularCone::new([0, 1])
        .axis(vector![1.0, 1.0])
    ...
    for k in 0..=100 {
        ...
        for x in -4..=4 {
            for y in -4..=4 {
                ...
                cone_project.project(&mut project);
        ...
        let component = rerun::Points2D::new(&original).with_colors([Color::from_rgb(255, 180, 0)]).with_radii([0.01]);
